...
class S3FileDeleteAdapterTest {
	...
	void deleteImage_타임라인_이미지를_삭제하면_원본과_람다가_생성한_리사이징_사이즈_전부를_삭제_요청한다() {
		...
		assertThat(objectKeys).containsExactlyInAnyOrder(
			"activity/1/timeline/original/timeline-image.jpg",
			"activity/1/timeline/small/timeline-image.jpg",
			"activity/1/timeline/medium/timeline-image.jpg",
			"activity/1/timeline/large/timeline-image.jpg"
		);
	}
	...
	void deleteImage_RECAP_이미지를_삭제하면_사이즈_구분_없이_단일_키만_삭제_요청한다() {
		...
		assertThat(objectKeys).containsExactly("activity/1/recap/recap-image.jpg");
	}
